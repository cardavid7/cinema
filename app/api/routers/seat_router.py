...
def create_seat(db: DBSession, seat: SeatCreate, user: CurrentUser):
    service = SeatService(db)
    return service.create(seat)

@router.put("/{seat_id}", response_model=Seat, status_code=200)
def update_seat(db: DBSession, seat_id: int, seat: SeatUpdate, user: CurrentUser):
    service = SeatService(db)
    return service.update(seat_id, seat)

@router.delete("/{seat_id}", status_code=204)
def delete_seat(db: DBSession, seat_id: int, user: CurrentUser):
    service = SeatService(db)
    return service.delete(seat_id)

@router.delete("/room/{room_id}", status_code=204)
def delete_all_seats_by_room_id(db: DBSession, room_id: int, user: CurrentUser):
    service = SeatService(db)
